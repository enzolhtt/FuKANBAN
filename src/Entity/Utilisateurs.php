...
class Utilisateurs implements UserInterface, PasswordAuthenticatedUserInterface
{
    ...
    #[ORM\OneToMany(mappedBy: 'NomCreateur', targetEntity: Projets::class)]
    ...
    public function addDateCreation(Projets $dateCreation): static
    {
        if (!$this->DateCreation->contains($dateCreation)) {
            $this->DateCreation->add($dateCreation);
            $dateCreation->setIdCreateur($this);
        }

        return $this;
    }

    public function removeDateCreation(Projets $dateCreation): static
    {
        if ($this->DateCreation->removeElement($dateCreation)) {
            // set the owning side to null (unless already changed)
            if ($dateCreation->getIdCreateur() === $this) {
                $dateCreation->setIdCreateur(null);
            }
        }
    ...
    public function removeProjet(Projets $projet): static
    {
        if ($this->projets->removeElement($projet)) {
            // set the owning side to null (unless already changed)
            if ($projet->getNomCreateur() === $this) {
                $projet->setNomCreateur(null);
            }
        }
